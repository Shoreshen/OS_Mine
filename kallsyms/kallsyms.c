...
int read_symbol(FILE *filp, symbol_entry *sym_entry)
{
	char string[100];
	int rc;

	rc = fscanf(filp,"%llx %c %99s\n",&sym_entry->address,&sym_entry->type,string);
	if(rc != 3) 
	{
		if (rc != EOF) {
	...
	memcpy(&sym_entry->symbol[0],&string[0],sym_entry->symbol_length);
	return 0;
}
...
void read_map(FILE *filp)
{
	...
	while(!feof(filp)) 
	{
		if(count >= size){
			size += 100;
			table = realloc(table,sizeof(symbol_entry) * size);
		}
...
int symbol_valid(symbol_entry *sym_entry)
{
